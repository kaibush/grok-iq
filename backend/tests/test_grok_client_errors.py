...
def test_is_quality_guard_unavailable_detects_config_gate():
    from app.integrations.grok2api.client import (
        IntegrationError,
        is_quality_guard_unavailable,
    )
    ...
    other = IntegrationError("timeout", status_code=503, error_code="upstream")

    assert is_quality_guard_unavailable(unavailable) is True
    assert is_quality_guard_unavailable(other) is False
    assert is_quality_guard_unavailable(RuntimeError("nope")) is False
...
def test_model_account_bind_window_message_explains_official_grok2api_limit():
    ...
    unavailable = model_account_bind_window_message(
        4725, quality_guard_unavailable=True
    )

    assert "账号 4725" in pinned
    assert "最新约 1000 个账号" in pinned
    assert "实际命中了账号 99" in pinned
    assert "批量设置出口" in pinned
    assert "qualityGuard.enabled: true" in pinned
    assert "没有可用出口节点" in missing
    assert "质量守护未开启" in unavailable
    assert "sidecar 容器可以不启动" in unavailable
